   ...
   public function createRideRequest(Request $request): JsonResponse
    {
     ...
     // 1. Check if user has carpool in status NOT completed and NOT cancelled
//$activeCarpool = $this->tripRequestRepository->getBy([
    //'customer_id' => auth('api')->id(),
  //  'type' => 'carpool',
//])->whereNotIn('status', ['completed', 'cancelled'])->first();

// If user has carpool that is active AND not just pending, block all creation
//if ($activeCarpool && $activeCarpool->status !== 'pending') {
  //  return response()->json(
    //    responseFormatter(DEFAULT_403, [
      //      'message' => 'Cannot create a ride because you have an ongoing carpool route.'
        //]), 403
    //);
//}

// 2. If user has a pending carpool, block creation of carpool only
...
if($request->is_carpool == true) {
    ...
    $route = CarpoolRoute::where('id', $request->carpool_route_id)
            ->with(['user', 'vehicle', 'trip']) // تأكد إنه بيجيب العلاقة
            ->first();
    ...
    // Get encoded polyline from carpool route
    if ($route->encoded_polyline) {
        ...
        if (!empty($find_drivers)) {
            \Log::info('=== STARTING DRIVER NOTIFICATIONS ===');
            \Log::info('Total drivers found: ' . count($find_drivers));
            
            $notify = [];
            foreach ($find_drivers as $key => $value) {
                \Log::info('Processing driver #' . $key . ': ' . json_encode([
                    'driver_id' => $value->id ?? 'null',
                    'user_id' => $value->user_id ?? 'null',
                    'has_user_object' => isset($value->user),
                    'user_object_type' => isset($value->user) ? get_class($value->user) : 'null',
                    'user_properties' => isset($value->user) ? array_keys(get_object_vars($value->user)) : []
                ]));
                
                if ($value->user?->fcm_token) {
                    \Log::info('✅ FCM token found for driver #' . $key . ': ' . substr($value->user->fcm_token, 0, 20) . '...');
                    $notify[$key]['user_id'] = $value->user->id;
                    $notify[$key]['trip_request_id'] = $final->id;
                } else {
                    \Log::warning('❌ No FCM token for driver #' . $key . ' - user object: ' . json_encode($value->user ?? 'null'));
                }
            }
            
            \Log::info('Drivers with FCM tokens: ' . count($notify));
            ...
            if (!empty($notify)) {
                \Log::info('🚀 Sending notifications to ' . count($notify) . ' drivers');
                
                // Send notifications directly instead of using SendPushNotificationJob
                foreach ($notify as $key => $notification_data) {
                    $driver = $find_drivers[$key];
                    if ($driver->user && $driver->user->fcm_token) {
                        try {
                            ...
                            \Log::info('✅ Notification sent to driver: ' . $driver->user->id . ' with token: ' . substr($driver->user->fcm_token, 0, 20) . '...');
                        } catch (Exception $e) {
                            \Log::error('❌ Failed to send notification to driver ' . $driver->user->id . ': ' . $e->getMessage());
                        }
                    }
                }
                ...
                \Log::warning('⚠️ No drivers with FCM tokens found!');
            }
            ...
            foreach ($find_drivers as $key => $value) {
                try {
                    ...
                    \Log::error('Pusher error for driver #' . $key . ': ' . $exception->getMessage());
                }
            }
            
            \Log::info('=== ENDING DRIVER NOTIFICATIONS ===');
        } else {
            \Log::warning('No drivers found in the area!');
        }
...
private function getResumeRide(bool $isCarpool = false): mixed
{
    $attributes = [
    ...
    $commonAttributes = [
        ...
        'withAvgColumn' => 'rating',
        'relations' => [
            'customer', 'driver', 'vehicleCategory', 'vehicleCategory.tripFares',
            'vehicle', 'coupon', 'time', 'coordinate', 'fee', 'tripStatus', 'zone',
            'vehicle.model', 'fare_biddings', 'parcel', 'parcelUserInfo',
            'customerReceivedReviews', 'driverReceivedReviews'
        ]
    ];

    if ($isCarpool) {
        $commonAttributes['whereInColumn'] = 'type';
        $commonAttributes['whereInValues'] = ['carpool'];
    }

    return $this->trip->getBy(
        column: 'id',
